...
/* make sure that barrier does not cause a busy spin */

void thread_fn(void *_arg)
{
    ABT_barrier *barrier = _arg;
...
int main(int argc, char **argv) 
{
    ...
    ABT_barrier_create(2, &barrier);

    /* launch a ULT on the new ES that will do nothing except sleep() */
    ABT_thread_create(pool2, thread_fn, &barrier, ABT_THREAD_ATTR_NULL, &tid);
